...
db.serialize(() => {
  db.run('CREATE TABLE IF NOT EXISTS seats (id INTEGER PRIMARY KEY AUTOINCREMENT, row INTEGER, col INTEGER, isBooked BOOLEAN)');
});
...
function initializeSeats(rows, cols) {
  db.serialize(() => {
    ...
    const stmt = db.prepare('INSERT INTO seats (row, col, isBooked) VALUES (?, ?, ?)');
...
function bookSeat(row, col) {
  return new Promise((resolve, reject) => {
    db.run('UPDATE seats SET isBooked = ? WHERE row = ? AND col = ?', [true, row, col], (err) => {
      if (err) {
        reject(err);
      } else {
...
function cancelSeat(row, col) {
  return new Promise((resolve, reject) => {
    db.run('UPDATE seats SET isBooked = ? WHERE row = ? AND col = ?', [false, row, col], (err) => {
      if (err) {
        reject(err);
      } else {
